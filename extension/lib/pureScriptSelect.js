...
const pureScriptSelect = (selector) => {
    ...
    let optionValues = {
        [document.querySelector(selector).getAttribute('id')]: JSON.parse(document.querySelector(selector).getAttribute('data-multiSelect'))
    };


    let isMax = {
        [document.querySelector(selector).getAttribute('id')]: JSON.parse(document.querySelector(selector).getAttribute('data-max'))
    };
    ...
    selectors.forEach((item, index) => {
        ...
        function multiSelects(){
            let defaultValues = {
                [document.querySelector(selector).getAttribute('id')]: document.querySelector(selector).getAttribute('data-default') ? JSON.parse(document.querySelector(selector).getAttribute('data-default')) : []
            };
            const arraySelector = item.getAttribute('id');            
            const hiddenInput = item.querySelector('input[type="hidden"]');

            ///
            hiddenInput.value = JSON.stringify(defaultValues['multiSelect']);
            ///
